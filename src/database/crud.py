...
class TeamCRUD:
    ...
    @staticmethod
    def create_team_from_analysis(
        analysis_result: Dict[str, Any], article_url: str = ""
    ) -> Team:
    ...
    def get_team_statistics() -> Dict[str, Any]:
        ...
        try:
            ...
            regulations = (
                session.query(Team.regulation, func.count(Team.regulation))
            ...
            popular_pokemon = (
                session.query(Pokemon.name, func.count(Pokemon.name))
...
class BookmarkCRUD:
    ...
    @staticmethod
    def create_bookmark(
        article_url: str, title: str = "", summary: str = "", category: str = "General"
    ) -> Bookmark:
        """Create a bookmark"""
        session = get_session()
        try:
            ...
            existing = (
                ...
                .filter(Bookmark.article_url == article_url)
                .first()
            )
            if existing:
                return existing
            ...
            bookmark = Bookmark(
                article_url=article_url, title=title, summary=summary, category=category
            )
    ...
    @staticmethod
    def update_bookmark_rating(
        bookmark_id: int, rating: float, notes: str = ""
    ) -> bool:
    ...
    def create_speed_tier(pokemon_name: str, base_speed: int, **kwargs) -> SpeedTier:
        ...
        try:
            speed_tier = SpeedTier(
                pokemon_name=pokemon_name, base_speed=base_speed, **kwargs
            )
...
class DamageCalculationCRUD:
    ...
    @staticmethod
    def get_calculations_for_pokemon(
        pokemon_name: str, as_attacker: bool = True
    ) -> List[DamageCalculation]:
        """Get damage calculations involving a Pokemon"""
        session = get_session()
        try:
            if as_attacker:
                return (
                    session.query(DamageCalculation)
                    .filter(DamageCalculation.attacker_name == pokemon_name)
                    .order_by(desc(DamageCalculation.created_at))
                    .all()
                )
            else:
                return (
...
def initialize_database():
    ...
    try:
        for name, base, pos, neu, neg, tier in sample_speed_tiers:
            existing = (
                session.query(SpeedTier).filter(SpeedTier.pokemon_name == name).first()
            )
            if not existing:
                speed_tier = SpeedTier(
                    pokemon_name=name,
