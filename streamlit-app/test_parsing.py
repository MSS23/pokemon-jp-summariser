...
# Test script for Pokemon parsing
import logging
...
def parse_pokemon_alternative(summary):
    ...
    logger.info(f"Starting alternative parsing with {len(lines)} lines")
    ...
    for i, line in enumerate(lines):
        ...
        if any(alt_patterns):
            ...
            if current_pokemon:
                teams.append(current_pokemon)
                logger.info(f"Added Pokemon (alt): {current_pokemon['name']}")
            ...
            logger.debug(f"Found Pokemon (alt): {name_part}")
            current_pokemon = {
                "name": name_part,
                "ability": "Not specified",
                "item": "Not specified",
            ...
            if line.startswith("- Ability:"):
                ...
                if "/" in moves_text:
                    current_pokemon["moves"] = [
                        move.strip() for move in moves_text.split("/") if move.strip()
                    ]
                elif "," in moves_text:
                    current_pokemon["moves"] = [
                        move.strip() for move in moves_text.split(",") if move.strip()
                ...
                while (
                    j < len(lines)
                    and lines[j].strip()
                    and not lines[j].strip().startswith("**Pokémon")
    ...
    if current_pokemon:
        teams.append(current_pokemon)
        logger.info(f"Added final Pokemon: {current_pokemon['name']}")
