...
def show_login_form():
    ...
    with st.form("login_form"):
        username = st.text_input(
            "Username", placeholder="Enter your username", help="Your unique username"
        )
        ...
        if login_button:
            ...
            if username.lower() == "testuser":
                st.info(
                    f"🔍 Debug: Attempting login for testuser with password: {password[:3]}***"
                )
            ...
            if success:
                ...
                if username.lower() == "testuser":
                    st.info(
                        "💡 Test account credentials: Username: 'testuser', Password: 'testpass123'"
                    )
                return False
...
def show_user_profile():
    ...
    with col4:
        ...
        st.metric(
            "🚀 Total Logins", login_count, help="Number of times you've logged in"
        )
    ...
    with st.expander("🎨 Preferences", expanded=False):
        ...
        notifications = st.checkbox(
            "Enable Notifications", value=preferences.get("notifications", True)
        )

        if st.button("💾 Save Preferences"):
            new_preferences = {"theme": theme, "notifications": notifications}
    ...
    with st.expander("📋 Account Information", expanded=False):
        ...
        st.write(
            f"**Last Login:** {user_info.get('last_login', 'Unknown')[:19].replace('T', ' ')}"
        )
