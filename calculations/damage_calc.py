...
class DamageCalculator:
    ...
    TYPE_CHART = {
        ...
        'Dark': {
            'Fighting': 0.5, 'Psychic': 2, 'Bug': 0.5, 'Rock': 0.5, 'Ghost': 2, 'Dark': 0.5, 'Fairy': 0.5
        },
        'Steel': {
    ...
    @classmethod
    def get_type_effectiveness(cls, move_type: str, defender_type1: str, defender_type2: str = None) -> float:
        ...
        if defender_type2 and move_type in cls.TYPE_CHART and defender_type2 in cls.TYPE_CHART[move_type]:
            effectiveness *= cls.TYPE_CHART[move_type][defender_type2]
        
        return effectiveness
    
    @classmethod
    def calculate_stat(cls, base: int, iv: int, ev: int, level: int, nature_modifier: float = 1.0) -> int:
        """Calculate actual stat from base/IV/EV"""
        if base == 1:  # HP calculation
            return math.floor((2 * base + iv + math.floor(ev / 4)) * level / 100) + level + 10
        else:
            return math.floor((math.floor((2 * base + iv + math.floor(ev / 4)) * level / 100) + 5) * nature_modifier)
    ...
    def calculate_damage_range(
        ...
        # Apply modifiers
        attack = math.floor(attack * modifiers.ability_attacker * modifiers.item_attacker)
        defense = math.floor(defense * modifiers.ability_defender * modifiers.item_defender)
        ...
        # Type effectiveness
        type_effectiveness = cls.get_type_effectiveness(move.type, defender_types[0], 
        ...
        final_modifier = modifiers.weather * modifiers.terrain * stab * type_effectiveness
...
COMMON_POKEMON_STATS = {
    "Miraidon": {"hp": 100, "attack": 85, "defense": 100, "sp_attack": 135, "sp_defense": 115, "speed": 135},
    "Koraidon": {"hp": 100, "attack": 135, "defense": 115, "sp_attack": 85, "sp_defense": 100, "speed": 135},
    "Flutter Mane": {"hp": 55, "attack": 55, "defense": 55, "sp_attack": 135, "sp_defense": 135, "speed": 135},
    "Iron Valiant": {"hp": 74, "attack": 130, "defense": 90, "sp_attack": 120, "sp_defense": 60, "speed": 116},
    "Dragapult": {"hp": 88, "attack": 120, "defense": 75, "sp_attack": 100, "sp_defense": 75, "speed": 142},
    "Landorus-Therian": {"hp": 89, "attack": 145, "defense": 90, "sp_attack": 105, "sp_defense": 80, "speed": 91},
    "Incineroar": {"hp": 95, "attack": 115, "defense": 90, "sp_attack": 80, "sp_defense": 90, "speed": 60},
    "Raging Bolt": {"hp": 125, "attack": 73, "defense": 91, "sp_attack": 137, "sp_defense": 89, "speed": 75},
    "Chien-Pao": {"hp": 80, "attack": 120, "defense": 80, "sp_attack": 90, "sp_defense": 65, "speed": 135},
    "Urshifu-Rapid": {"hp": 100, "attack": 130, "defense": 100, "sp_attack": 63, "sp_defense": 60, "speed": 97},
}
